...
def Train_modelPROB(model, train_loader, val_loader, criterion, optimizer, scheduler=None, num_epochs=10, device='cuda', is_binary=True, patience=5, oTBLogger=None, l1_lambda=None):
    history = {'train_loss': [], 'val_loss': [], 'train_score': [], 'val_score': [], 'train_precision': [], 'train_recall': [], 'train_f1': [],
    'val_precision': [], 'val_recall': [], 'val_f1': [], 'epoch_time': [], 'learning_rate': [], 'last_saved_epoch': None}
    ...
    for epoch in range(num_epochs):
        ...
        train_losses = []
        all_train_labels = []
        all_train_probs = []  # <-- Changed to store probabilities
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            # Store raw probabilities for analysis (without rounding)
            if is_binary:
                probs = torch.sigmoid(outputs).detach().cpu().numpy().flatten()  # <-- Store probabilities instead of rounding
            else:
                probs = torch.softmax(outputs, dim=1).detach().cpu().numpy()
            ...
            # Print batch iteration info
            print(f'\rTrain Epoch {epoch+1}/{num_epochs} - Batch {batch_idx+1}/{total_train_batches} - Loss: {loss.item():.4f}', end='')

        print('', end='\r')

        train_loss = sum(train_losses) / len(train_losses)
        train_score = accuracy_score(all_train_labels, (np.array(all_train_probs) >= 0.5).astype(int))  # Accuracy based on threshold 0.5

        # Plot the distribution of predicted probabilities for training data
        plt.figure()
        plt.hist(all_train_probs, bins=20, range=(0, 1), alpha=0.7, color='blue', label='Train Probs')
        plt.title(f'Predicted Probabilities Distribution - Train Epoch {epoch + 1}')
        plt.xlabel('Probability')
        plt.ylabel('Frequency')
        plt.legend()
        plt.show()

        # Calculate precision, recall, f1 for train
        try:
            average_method = 'binary' if is_binary else 'macro'
            train_precision = precision_score(all_train_labels, (np.array(all_train_probs) >= 0.5).astype(int), average=average_method, zero_division=0)
            train_recall = recall_score(all_train_labels, (np.array(all_train_probs) >= 0.5).astype(int), average=average_method, zero_division=0)
            train_f1 = f1_score(all_train_labels, (np.array(all_train_probs) >= 0.5).astype(int), average=average_method, zero_division=0)
        except ZeroDivisionError:
            train_precision, train_recall, train_f1 = 0.0, 0.0, 0.0
            print("ZeroDivisionError encountered in calculating train metrics. Setting it to 0.0")

        # ======= Validation Phase =======
        model.eval()
        val_losses = []
        all_val_labels = []
        all_val_probs = []  # <-- Changed to store probabilities
        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(val_loader):
                ...
                # Store raw probabilities for analysis
                if is_binary:
                    probs = torch.sigmoid(outputs).detach().cpu().numpy().flatten()  # <-- Store probabilities instead of rounding
                else:
                    probs = torch.softmax(outputs, dim=1).detach().cpu().numpy()
                ...
                # Print batch iteration info
                print(f'\rValidation Epoch {epoch+1}/{num_epochs} - Batch {batch_idx+1}/{total_val_batches} - Loss: {loss.item():.4f}', end='')
            print('', end='\r')

        val_loss = sum(val_losses) / len(val_losses)
        val_score = accuracy_score(all_val_labels, (np.array(all_val_probs) >= 0.5).astype(int))  # Validation accuracy based on 0.5 threshold

        # Plot the distribution of predicted probabilities for validation data
        plt.figure()
        plt.hist(all_val_probs, bins=20, range=(0, 1), alpha=0.7, color='green', label='Val Probs')
        plt.title(f'Predicted Probabilities Distribution - Val Epoch {epoch + 1}')
        plt.xlabel('Probability')
        plt.ylabel('Frequency')
        plt.legend()
        plt.show()
        ...
        try:
            val_precision = precision_score(all_val_labels, (np.array(all_val_probs) >= 0.5).astype(int), average=average_method, zero_division=0)
            val_recall = recall_score(all_val_labels, (np.array(all_val_probs) >= 0.5).astype(int), average=average_method, zero_division=0)
            val_f1 = f1_score(all_val_labels, (np.array(all_val_probs) >= 0.5).astype(int), average=average_method, zero_division=0)
        except ZeroDivisionError:
            val_precision, val_recall, val_f1 = 0.0, 0.0, 0.0
            print("ZeroDivisionError encountered in calculating val metrics. Setting them to 0.0")
        ...
        print(f"Epoch {epoch + 1}| Train Loss: {train_loss:.4f}      | Val Loss: {val_loss:.4f}      | Train Score: {train_score:.4f}  | Val Score: \033[1;33m{val_score:.4f}\033[0m  | Learning Rate: {current_lr:.6f}, Time: {epoch_time:.2f}s | ", end='')
        print(f'\nEpoch {epoch + 1}| Train Precision: {train_precision:.4f} | Val Precision: \033[1m{val_precision:.4f}\033[0m | Train Recall: {train_recall:.4f} | Val Recall: \033[1m{val_recall:.4f}\033[0m | F1 Train Score: {train_f1:.4f} | F1 Val Score: {val_f1:.4f}', end='')
        ...
        if val_loss < best_val_loss:
            ...
            history['last_saved_epoch'] = last_saved_epoch

            save_path = f'Best_Model_{epoch + 1}.pth'
            save_dict = {
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'epoch': epoch + 1,
            ...
            print(f"--> Saved as best model", flush=True)
        else:
            patience_counter += 1
    ...
    print(f"Best model saved at epoch {last_saved_epoch}.")
